<script>
  ...
  oppositePlayers = [1, 3, 5].includes(+currentPlayer) ? [2, 4, 6] : [1, 3, 5];
  ...
  async function destroy() {
    ...
    try {
      ...
      if (obj.active) {
        let data = {
          active: [...obj.active.filter((id) => id !== currentPlayer)],
        };
  ...
  function showOptions(e) {
    ...
    if (in_pit_drop) {
      ...
      if (confirm("Are you sure you want to drop this pit?"))
        dropPit(room, currentPlayer, findPit(e.target.alt), details);
    } else {
...
<div
  class="container-fluid {inplay ? 'active' : ''}"
  style="{inplay ? `background: ${data.background_color};` : ''} {loading
    ? `display: none;`
    : ''}"
>
  <div class="row">
    ...
    <div class="col-md-6 d-flex" id="players-panel">
      {#each Object.keys(players) as t}
        {#if t !== currentPlayer}
          <div
            class="player-block{turn == t ? ' active' : ''}{!active.includes(+t)
              ? ' striped'
              : ''}"
          >
        ...
        {#each oppositePlayers as number}
          <div class="form-check form-check-inline player-option">
            <input
              ...
              bind:group={callee}
            />
            <label for={number} class="form-check-label">Player {number}</label>
          </div>
        {/each}
      </div>
    {/if}
            ...
            {#each players[currentPlayer] as id}
              <div class="card-container">
                <img
                  ...
                  on:mouseenter={(e) =>
                    (timeout = window.setTimeout(() => showDrop(e), 1500))}
                  on:mouseleave={(e) => {
